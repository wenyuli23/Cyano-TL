...
for target in ['growth_rate', 'product_titer', 'production_rate', 'OD']:
    ...
    predictions_df.to_csv(f'./predictions/{target}_pred.csv', index=False)
    
    print(f'Saved predictions for {target} to ./predictions/{target}_pred.csv')
